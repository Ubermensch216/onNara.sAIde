...
const ko = {
  ...
  'inbox.markedRead': '설정에 따라 {n}건을 열어 열람 처리했습니다',
  ...
  'opt.inbox.readMark': '브리핑한 문서를 열어 열람 처리',
  'opt.inbox.readDesc': '기본값은 목록 표만 읽습니다. 본문을 열지 않으므로 문서는 미열람 그대로 남습니다. 브리핑 전후로 열람 상태가 바뀐 문서가 있으면 그 건수를 공유/공람 탭에 알립니다.',
  'opt.inbox.readWarn': '문서를 열면 온나라 서버에 열람 기록이 남으며, 확장은 이를 되돌릴 수 없습니다. 받은문서 목록을 보고 있는 탭에서 확인할 때만 동작합니다.',
  'opt.inbox.openLimit': '한 번에 열 문서 수',
  'opt.inbox.openLimitDesc': '문서 한 건마다 몇 초가 걸립니다. 상한을 넘는 문서는 열지 않습니다.',
...
const en = {
  ...
  'inbox.markedRead': 'Opened {n} documents and marked them read, per your setting',
  ...
  'opt.inbox.readMark': 'Open briefed documents and mark them read',
  ...
  'opt.inbox.readWarn': 'Opening a document records it as read on the onNara server, and the extension cannot undo that. Works only when checking from a tab showing the received-documents list.',
  'opt.inbox.openLimit': 'Documents to open at once',
  'opt.inbox.openLimitDesc': 'Each document takes a few seconds. Documents beyond the limit are not opened.',
