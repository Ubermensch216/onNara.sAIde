...
const ko = {
  ...
  'opt.inbox.on': '아침 브리핑',
  'opt.inbox.onDesc':
    '켜면 정한 시각 이후 브라우저를 처음 쓸 때 하루 한 번, 온나라 받은문서 목록을 읽어 오늘 볼 문서를 갈래로 나눕니다. 브라우저가 실행 중이고 온나라에 로그인돼 있어야 동작하며, 사이드패널은 닫혀 있어도 됩니다.',
  'opt.inbox.hour': '브리핑 시각',
  'opt.inbox.hourDesc': '이 시각 이후 브라우저를 처음 쓸 때 그날의 브리핑을 한 번 수행합니다. 그 밖의 때에는 공유/공람 탭의 "지금 확인"이나 @브리핑으로 직접 확인합니다.',
  ...
  'opt.inbox.readDesc': '기본값은 목록 표만 읽습니다. 본문을 열지 않으므로 문서는 미열람 그대로 남고, 브리핑 전후로 열람 상태가 바뀌지 않았음을 공유/공람 탭에 건수로 보여 줍니다.',
...
const en = {
  ...
  'inbox.target': 'Target: {name}',
  'inbox.change': 'Choose again',
  'inbox.never': 'Not checked yet',
  'inbox.lastRun': 'Checked {time} · {n} rows',
  'inbox.newCount': '{n} new',
  'inbox.keepUnread': 'Read state unchanged · {n} still unread',
  ...
  'inbox.readUnknown': 'This list does not report read state',
  ...
  'opt.inbox.on': 'Morning briefing',
  'opt.inbox.onDesc':
    'Once a day, the first time you use the browser after the chosen hour, reads the onNara received-documents list and sorts what to read today. The browser must be running and signed in to onNara; the side panel may stay closed.',
  'opt.inbox.hour': 'Briefing hour',
  'opt.inbox.hourDesc': 'Runs once on the first browser use after this hour. At other times, use "Check now" in the Shared docs tab or the @inbox command.',
  ...
  'opt.inbox.readDesc': 'By default only the list table is read. Documents are never opened, so they stay unread, and the Shared docs tab reports the unchanged read state as a count.',
