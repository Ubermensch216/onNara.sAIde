...
export interface InboxDoc extends InboxRow {
  ...
  markedReadAt?: number;
  /**
   * 일정을 만들려고 본문을 열어 열람 처리된 시각.
   *
   * ★ 앞의 두 시각과 또 나눠 둔다. 이것은 사용자가 "일정 등록"을 누르며 열람을 감수한
   *   경우다 — 읽으려고 연 것도, 설정이 시킨 것도 아니다. 어느 이유로 문서가 열렸는지
   *   섞이면, 미열람을 지킨다는 이 기능의 약속을 나중에 검증할 수 없다.
   */
  taskReadAt?: number;
