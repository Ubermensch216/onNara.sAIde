...
class SaideDB extends Dexie {
   ...
   *
   * ★ ① 판본은 되돌리지 않는다.
   *   판본을 낮춘 빌드도 DB 자체는 연다(Dexie 4는 VersionError를 던지지 않는다). 다만
   *   **그 빌드가 선언하지 않은 표는 없는 것이 된다** — 사용자 DB가 v5인데 v4까지만 선언한
   *   빌드가 깔리면 inboxDocs를 만지는 순간 `InvalidTableError`로 던진다. 데이터는
   *   IndexedDB에 그대로 남아 있어 판본을 다시 올리면 돌아오지만, 롤백해 있는 동안 그
   *   기능은 오류로 멈춘다. 되돌려야 하면 번호를 낮추지 말고 더 올린 새 판본으로 내려간다.
   *   [migration.test.ts]가 이 동작을 그대로 고정해 두었다.
   *
   * ★ ② 표는 지우지 않는다.
   *   `stores({ x: null })`은 그 표를 **삭제**한다. 쓰지 않게 된 표라도 남겨 둔다 —
   *   지우는 순간 그 안의 것은 백업 파일에만 남고, 판본을 되돌릴 길이 사라진다.
   *
   * ★ ③ 새 필드는 색인 없이 더한다.
   *   Dexie의 `stores()`에 적는 것은 **색인**이지 필드가 아니다. 인터페이스에 필드를
   *   더하는 데에는 새 판본이 필요 없고, 새 판본을 만들지 않는 편이 안전하다
   *   (StoredMessage의 steps·origin·cached가 그렇게 들어왔다).
   *   범위 조회가 필요한 필드만 색인을 만들고, 그때는 판본을 하나 올린다.
   *
   * ★ ④ 새 판본은 **더하기만** 한다.
   *   기존 표의 색인을 바꾸면 Dexie가 그 표를 다시 만든다. 색인을 고쳐야 하면
   *   `upgrade()`로 옮겨 담는 길을 함께 적어, 옮기지 못한 행이 없는지 시험으로 고정한다.
   *
   * ★ ⑤ 이 규칙은 [migration.test.ts]가 지킨다 — 옛 판본 DB를 열어 보존을 확인하고,
   *   선언된 표가 사라지면 실패한다.
   */
  constructor() {
    super('saide');
