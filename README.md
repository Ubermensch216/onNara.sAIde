...
| **/기억** | `/recall`, `/memory` | 저장된 기억(과거에 읽은 문서)에서 찾아 대화에 붙입니다. 설정에서 **기억을 켠 경우에만** 동작합니다.<br>*(예: `/기억 예산 편성 지침`)* | 임베딩만 |
...
* **성능 기록 · 프리셋 편집기 · 기억 관리 · 기한 알림 · 페이지 접근 권한 · 표시(테마/언어)** 항목이 같은 화면에 이어집니다.
...
* **로컬 벡터 데이터베이스 (Dexie / IndexedDB)**:
  ...
  * 로컬 임베딩 모델(`bge-m3`)로 질의어와 가까운 조각을 벡터 검색(코사인 유사도)으로 찾습니다. 키워드 색인과의 하이브리드 검색(RRF)은 계획서 §7.3의 후속 작업입니다.
...
# 전체 단위 및 통합 테스트 실행 (62개 파일, 715개 테스트 · 2026-09-20 확인)
npm test
